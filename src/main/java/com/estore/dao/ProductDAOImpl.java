...
public class ProductDAOImpl implements ProductDAO {
	...
	public List<Product> findAll() {
		String hql="FROM Product"; //WHERE quantity > 0 AND available <> false
		Session session = factory.getCurrentSession();
	...
	public List<Product> findByKeywords(String keywords) {
		String hql="FROM Product p WHERE p.name LIKE :kw OR p.category.nameVN LIKE :kw";
		Session session = factory.getCurrentSession();
		TypedQuery<Product> query = session.createQuery(hql,Product.class);
	...
	public List<Product> findBySpecial(Integer id) {
		...
		case 1: // bán chạy
			...
			break;
		case 2: // xem nhiều
			hql="FROM Product p ORDER BY p.views DESC";
			break;
		case 3: // giảm giá
